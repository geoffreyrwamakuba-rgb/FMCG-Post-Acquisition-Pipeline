...
df_silver = df_silver.withColumn(
    "gross_price_gbp",
    F.when(F.col("gross_price_gbp").rlike(r'^-?\d+(\.\d+)?$'), 
           F.when(F.col("gross_price_gbp").cast("double") < 0, -1 * F.col("gross_price_gbp").cast("double"))
            .otherwise(F.col("gross_price_gbp").cast("double")))
    .otherwise(0)
)
...
df_silver = df_silver.withColumn("gross_price_gbp", round(col("gross_price_gbp"), 2))
df_silver.show(10)
